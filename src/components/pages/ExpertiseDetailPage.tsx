...
export function ExpertiseDetailPage({ detail }: { detail: ExpertiseDetail }) {
  ...
  const relevantPeople = lawyers.filter((person) => person.expertise.includes(detail.title)).slice(0, 2)
  const displayedPeople = relevantPeople.length > 0 ? relevantPeople : lawyers.slice(0, 2)
  const representativeWork = relevantPeople.flatMap((person) => person.representativeWorks).slice(0, 6)
  ...
  return (
    <>
      <section className="relative overflow-hidden bg-navy px-[clamp(22px,7vw,118px)] pt-[clamp(46px,6vw,72px)] pb-[clamp(72px,9vw,118px)] text-ivory dark:bg-[#030d1d]" aria-labelledby="expertise-detail-heading">
        ...
        <div className="absolute right-[8%] bottom-0 h-16 w-px bg-teal" aria-hidden="true" />
      </section>
      ...
      <section className="grid gap-14 bg-ivory px-[clamp(22px,7vw,118px)] py-[clamp(76px,9vw,118px)] dark:bg-[#08172a] lg:grid-cols-[.55fr_1.15fr] lg:gap-[clamp(80px,10vw,165px)]" id="overview">
        <div data-reveal="left">
          ...
          <p className="mt-6 max-w-[410px] text-[13px] leading-6 text-slate dark:text-paper/78">{detail.overview}</p>
        </div>
        <div data-reveal="right" data-reveal-delay="1">
          ...
          <div className="mt-10 grid gap-8 text-[15px] leading-7 text-slate dark:text-paper/78 md:grid-cols-2">
            ...
            <p className="m-0">Ninewells works across its Nigerian practices from the outset, giving decision-makers a coherent view without unnecessary layers or delay.</p>
          </div>
        </div>
      </section>

      <section className="grid bg-paper dark:bg-[#071224] lg:grid-cols-[.82fr_1.18fr]" id="capabilities">
        <div className="bg-navy-deep px-[clamp(22px,7vw,92px)] py-[clamp(76px,9vw,108px)] text-ivory dark:bg-[#020914]">
          <Eyebrow className="text-teal-light">How we help</Eyebrow>
          <h2 className="mt-6 mb-0 font-serif text-[clamp(44px,5vw,68px)] leading-[.98] font-normal tracking-[-.04em]">Capability built around the decision.</h2>
          <p className="mt-8 max-w-[420px] text-[14px] leading-7 text-white/75">We assemble the expertise a matter needs, with a senior team that remains accountable for the whole picture.</p>
        </div>
        <div className="px-[clamp(22px,7vw,92px)] py-[clamp(58px,8vw,98px)]">
          <div className="border-t border-navy dark:border-paper">
            {detail.capabilities.map((capability, index) => <div className="grid min-h-[78px] grid-cols-[42px_1fr_24px] items-center border-b border-navy/15 text-navy dark:border-white/15 dark:text-paper" key={capability}><span className="text-[10px] font-semibold text-teal-dark dark:text-teal-light">0{index + 1}</span><strong className="font-serif text-[21px] font-normal">{capability}</strong><Check className="text-teal-dark dark:text-teal-light" size={18} /></div>)}
          </div>
        </div>
      </section>

      <section className="bg-navy px-[clamp(22px,7vw,118px)] py-[clamp(76px,9vw,110px)] text-ivory dark:bg-[#030d1d]" aria-labelledby="experience-heading">
        <div className="grid gap-12 lg:grid-cols-[.4fr_1.2fr] lg:gap-20">
          <div><Eyebrow className="text-teal-light">Experience / Approach</Eyebrow><p className="mt-5 text-[11px] leading-5 text-white/80">Selected experience from the lawyers associated with this practice.</p></div>
          <div id="experience-heading">
            <p className="m-0 font-serif text-[clamp(31px,4vw,52px)] leading-[1.08]">Senior judgement, regulatory awareness and commercially astute advice for complex Nigerian and cross-border mandates.</p>
            {representativeWork.length ? <ol className="mt-10 grid list-none gap-x-10 border-t border-white/25 p-0 md:grid-cols-2">{representativeWork.map((item, index) => <li className="grid grid-cols-[38px_1fr] border-b border-white/20 py-5 text-[12px] leading-5 text-white/85" key={item}><span className="font-semibold text-teal-light">{String(index + 1).padStart(2, '0')}</span>{item}</li>)}</ol> : <p className="mt-8 max-w-[720px] text-[13px] leading-6 text-white/75">Representative matters for this practice are available on request. Contact the team for relevant experience.</p>}
          </div>
        </div>
        <div className="mt-16 grid border-t border-white/25 sm:grid-cols-3">
          {[['NG', 'Nigerian market insight'], ['SR', 'senior-led judgement'], ['01', 'integrated point of view']].map(([value, label]) => <div className="border-b border-white/20 py-7 sm:border-r sm:border-b-0 sm:pl-7 sm:first:pl-0 sm:last:border-r-0" key={label}><strong className="block font-serif text-[50px] font-normal text-teal-light">{value}</strong><span className="text-[11px] text-white/85">{label}</span></div>)}
        </div>
      </section>
      ...
      <section className="grid gap-12 border-t border-navy/15 bg-paper px-[clamp(22px,7vw,118px)] py-[clamp(70px,8vw,100px)] dark:border-white/15 dark:bg-[#071224] lg:grid-cols-[.55fr_1.15fr]" aria-labelledby="expertise-insights-heading">
        <div><Eyebrow>Related perspectives</Eyebrow><h2 className="mt-5 mb-0 font-serif text-[clamp(38px,4.5vw,58px)] leading-none font-normal text-navy dark:text-paper" id="expertise-insights-heading">Thinking in context.</h2></div>
        <div className="border-t border-navy dark:border-paper">
          {insightItems.slice(0, 2).map((item, index) => <a className="group grid min-h-[92px] grid-cols-[42px_1fr_24px] items-center border-b border-navy/15 text-navy no-underline dark:border-white/15 dark:text-paper" href={item.href} target="_blank" rel="noreferrer" key={item.title}><span className="text-[10px] font-semibold text-teal-dark dark:text-teal-light">0{index + 1}</span><span><small className="mb-2 block text-[9px] font-semibold uppercase tracking-[.1em] text-slate dark:text-paper/75">{item.category} · {item.date}</small><strong className="font-serif text-[20px] font-normal">{item.title}</strong></span><ArrowRight className="transition-transform group-hover:translate-x-1" size={17} /></a>)}
        </div>
      </section>
