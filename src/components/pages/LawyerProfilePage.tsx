...
export function LawyerProfilePage({ person }: { person: Person }) {
  ...
  const navigation = [
    ...
    ...(person.representativeWorks.length ? [['Representative work', 'representative-work']] : []),
    ...
    <>
      <section className="grid min-h-[760px] border-b border-navy/15 bg-ivory dark:border-white/15 dark:bg-[#08172a] lg:grid-cols-[1.08fr_.92fr]" aria-labelledby="profile-heading">
        <div className="flex flex-col px-[clamp(22px,7vw,118px)] pt-[clamp(42px,6vw,72px)] pb-[clamp(56px,7vw,84px)]" data-reveal="left">
          ...
          {person.title ? <p className="mt-5 max-w-[610px] text-[12px] leading-5 font-semibold text-teal-dark dark:text-teal-light">{person.title}</p> : null}
          <div className="mt-auto grid gap-3 border-t border-navy/20 pt-7 dark:border-white/20 sm:grid-cols-2 lg:mt-16">
            <a className="inline-flex min-h-11 items-center gap-3 text-[12px] font-semibold text-navy no-underline dark:text-paper" href={`mailto:${person.email}`}><EnvelopeSimple className="text-teal-dark dark:text-teal-light" size={20} /> {person.email}</a>
            <a className="inline-flex min-h-11 items-center gap-3 text-[12px] font-semibold text-navy no-underline dark:text-paper" href={`tel:${person.mobile.replace(/\s/g, '')}`}><Phone className="text-teal-dark dark:text-teal-light" size={20} /> {person.mobile}</a>
            <a className="inline-flex min-h-11 items-center gap-3 text-[12px] font-semibold text-navy no-underline dark:text-paper" href={person.sourceUrl} target="_blank" rel="noreferrer"><ArrowSquareOut className="text-teal-dark dark:text-teal-light" size={20} /> {person.sourceLabel}</a>
          </div>
        </div>
        <figure className="relative m-0 min-h-[560px] overflow-hidden bg-[#e5e9ec]" data-reveal="fade" data-reveal-delay="1">
          <img className="h-full w-full object-cover" src={person.image} style={{ objectPosition: person.imagePosition }} alt={`${person.name}, ${person.position} at Ninewells`} />
          <figcaption className="absolute right-0 bottom-0 bg-navy-deep px-6 py-4 text-[9px] font-semibold uppercase tracking-[.14em] text-ivory">{person.teamType} / Ninewells</figcaption>
        </figure>
      </section>
      ...
      <section className="grid gap-14 bg-paper px-[clamp(22px,7vw,118px)] py-[clamp(76px,9vw,118px)] dark:bg-[#071224] lg:grid-cols-[.45fr_1.15fr] lg:gap-[clamp(70px,10vw,170px)]">
        <div className="self-start lg:sticky lg:top-14">
          ...
          <nav className="mt-8 flex flex-col border-t border-navy/20 text-[12px] font-semibold text-navy dark:border-white/20 dark:text-paper" aria-label="On this page">
            {navigation.map(([label, id], index) => <a className="grid min-h-12 grid-cols-[32px_1fr] items-center border-b border-navy/15 no-underline dark:border-white/15" href={`#${id}`} key={id}><span className="text-[9px] text-teal-dark dark:text-teal-light">{String(index + 1).padStart(2, '0')}</span>{label}</a>)}
          </nav>
        </div>
        <div data-reveal="up">
          <div id="overview">
            <h2 className="m-0 max-w-[860px] font-serif text-[clamp(34px,4vw,55px)] leading-[1.08] font-normal tracking-[-.035em] text-navy dark:text-paper">{person.bio}</h2>
          ...
          {person.expertise.length ? <ProfileList title="Practice areas" id="expertise" items={person.expertise} linkItems /> : null}
          {person.industries.length ? <ProfileList title="Industries and functions" id="industries" items={person.industries} /> : null}
          {person.representativeWorks.length ? <ProfileCollection eyebrow="Selected experience" title="Representative work" id="representative-work" items={person.representativeWorks} /> : null}

          <section className="mt-20 border-t border-navy pt-8 dark:border-paper" id="qualifications">
            <Eyebrow>Qualifications</Eyebrow>
            <div className={`mt-7 grid gap-10 ${person.professionalQualifications.length ? 'lg:grid-cols-2' : 'max-w-[720px]'}`}>
              <QualificationBlock title="Academic" items={person.academicQualifications} />
              {person.professionalQualifications.length ? <QualificationBlock title="Professional" items={person.professionalQualifications} /> : null}
            </div>
          </section>

          {person.honours.length ? <ProfileCollection eyebrow="Recognition" title="Honours" id="honours" items={person.honours} /> : null}
          {person.careerHighlights.length ? <ProfileCollection eyebrow="Career" title="Career highlights" id="career" items={person.careerHighlights} compact /> : null}
          <ProfileCollection eyebrow="Professional community" title="Memberships" id="memberships" items={person.memberships} compact />
...
function ProfileCollection({ eyebrow, title, id, items, compact = false }: { eyebrow: string; title: string; id: string; items: string[]; compact?: boolean }) {
  return (
    <section className="mt-20 border-t border-navy pt-8 dark:border-paper" id={id}>
      <Eyebrow>{eyebrow}</Eyebrow>
      <h2 className="mt-5 mb-0 font-serif text-[clamp(34px,4vw,52px)] leading-none font-normal text-navy dark:text-paper">{title}</h2>
      <ol className={`mt-8 grid list-none p-0 ${compact ? 'gap-x-10 md:grid-cols-2' : ''}`}>
        {items.map((item, index) => <li className="grid grid-cols-[42px_1fr] border-b border-navy/15 py-5 text-[14px] leading-6 text-slate dark:border-white/15 dark:text-paper/82" key={item}><span className="text-[10px] font-semibold text-teal-dark dark:text-teal-light">{String(index + 1).padStart(2, '0')}</span><span>{item}</span></li>)}
      </ol>
    </section>
  )
}

function QualificationBlock({ title, items }: { title: string; items: string[] }) {
  return (
    <div>
      <h3 className="m-0 font-serif text-[25px] font-normal text-navy dark:text-paper">{title}</h3>
      <ul className="mt-5 list-none border-t border-navy/15 p-0 dark:border-white/15">
        {items.map((item) => <li className="border-b border-navy/15 py-4 text-[13px] leading-6 text-slate dark:border-white/15 dark:text-paper/80" key={item}>{item}</li>)}
      </ul>
    </div>
  )
}
