  ...
  return (
    <section className="relative grid min-h-[850px] border-b border-navy/15 bg-ivory dark:border-white/15 dark:bg-[#08172a] lg:grid-cols-[minmax(450px,1.38fr)_minmax(340px,.92fr)]" id="about" aria-labelledby="hero-heading">
      <div className="flex min-w-0 flex-col px-[clamp(22px,6vw,106px)] pt-[clamp(38px,5vw,50px)] pb-11" data-reveal="left">
        ...
        <h1 className="m-0 max-w-[780px] font-serif text-[clamp(60px,9vw,126px)] leading-[.84] font-normal tracking-[-.055em] text-navy dark:text-paper" id="hero-heading">
          Clarity for<br />complex<br />decisions.
        </h1>
        <p className="mt-9 max-w-[540px] text-[15px] leading-7 text-slate dark:text-paper/85 lg:mt-10 lg:text-[16px]">Ninewells is a fully integrated Nigerian law firm advising businesses, institutions and investors on complex commercial, regulatory and high-stakes matters.</p>
        ...
        <div className="mt-14 grid max-w-[560px] grid-cols-[72px_1fr] border-t border-navy/15 pt-6 dark:border-white/15 lg:mt-auto lg:grid-cols-[100px_1fr]">
          ...
          <div className="border-l border-navy/15 pl-5 dark:border-white/15 lg:pl-6">
            <Eyebrow>Firm perspective</Eyebrow>
            <a className="mt-2 flex max-w-[350px] items-end justify-between gap-5 font-serif text-[18px] leading-tight text-navy no-underline dark:text-paper lg:text-[20px]" href="#insights">
              Experience. Innovation. Trust. <ArrowRight className="shrink-0" size={18} />
            </a>
          </div>
        </div>
      </div>
      <figure className="group relative m-0 min-h-[500px] min-w-0 overflow-hidden bg-stone dark:bg-[#101f33] lg:min-h-0" data-reveal="fade" data-reveal-delay="1">
        ...
        <figcaption className="absolute inset-x-0 bottom-0 flex flex-wrap items-center justify-between gap-2 bg-navy-deep/95 px-6 py-4 text-[9px] uppercase tracking-[.13em] text-ivory">
          ...
          <a className="text-white/85 underline-offset-4 hover:text-white hover:underline" href="https://unsplash.com/photos/a-large-bridge-over-a-large-body-of-water-xayCN6mrMZ8" target="_blank" rel="noreferrer">Photo: Tunde Buremo / Unsplash</a>
        </figcaption>
      </figure>
      <a className="absolute bottom-0 left-0 hidden size-[52px] items-center justify-center bg-teal text-navy-deep lg:flex" href="#expertise" aria-label="Scroll to our expertise"><ArrowDown size={21} /></a>
    </section>
  )
}
