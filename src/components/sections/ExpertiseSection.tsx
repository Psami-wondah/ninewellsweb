    ...
    <section className="grid gap-14 bg-paper px-[clamp(22px,7vw,118px)] py-[clamp(76px,9vw,112px)] dark:bg-[#071224] lg:grid-cols-[minmax(300px,.8fr)_minmax(500px,1.35fr)] lg:gap-[clamp(60px,8vw,150px)]" id="expertise" aria-labelledby="expertise-heading">
      ...
      <div className="border-t border-navy dark:border-paper" data-reveal="right" data-reveal-delay="1">
        {expertise.map(([number, title, description], index) => {
          ...
          return (
            <article className={`border-b border-navy/15 transition-colors dark:border-white/15 ${isOpen ? 'bg-teal/5 dark:bg-teal/10' : ''}`} key={title}>
              <button className="grid min-h-[72px] w-full grid-cols-[42px_1fr_26px] items-center border-0 bg-transparent text-left text-navy dark:text-paper lg:grid-cols-[58px_1fr_30px]" type="button" onClick={() => setOpenExpertise(isOpen ? null : index)} aria-expanded={isOpen}>
                <span className="text-[12px] font-semibold tracking-[.06em] text-teal-dark dark:text-teal-light">{number}</span>
                <span className="font-serif text-[21px] lg:text-[25px]">{title}</span>
